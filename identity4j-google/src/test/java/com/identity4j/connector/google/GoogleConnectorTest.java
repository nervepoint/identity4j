...
public class GoogleConnectorTest extends AbstractRestWebServiceConnectorTest{
	...
	public void itShouldThrowConnectorExceptionForBadPasswordPolicyOfLengthLessThan8OnChangePassword() {
		...
		//then ConnectorException should be thrown 
	}
	...
	public void itShouldSetPasswordForValidIdentityAndEnforcePasswordChangeOnCheckCredential() {
		...
		try {
			...
			try {
				assertPasswordChange(validIdentityName, validIdentityPassword,
						newPassword);
	...
	public void itShouldUpdateIdentityWithTheChangesPassedSpecificToRole() {
		...
		try {
			...
			googleIdentity.setRoles(new Role[]{connector.getRoleByName(testRoleEmail1),connector.getRoleByName(testRoleEmail2)});
			...
			Assert.assertEquals("Full name should be same as updated name",
					googleIdentity.getFullName(),
					googleIdentityFromSource.getFullName());
	...
	private void assertRoleIsPresent(Role[] roles, String principalName){
		for (Role role : roles) {
			if(role.getAttribute("email").equals(principalName)) return;
		}
