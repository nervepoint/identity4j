...
public class SshCommand extends Expect {
	...
	final static Log LOG = LogFactory.getLog(SshCommand.class);

	private boolean ok;
	private SshSession session;

	public SshCommand(SshClient ssh, String command) throws IOException, SshException, ChannelOpenException, ExpectTimeoutException {
		this(ssh, command, null, null, null);
	}
	
	public SshCommand(SshClient ssh, String command, String sudoCommand, String sudoPrompt, String sudoPassword) throws IOException, SshException, ChannelOpenException, ExpectTimeoutException {
...
//		session = ssh.openSessionChannel(10000);
	...
	public int drainAndWaitForExit() throws IOException {
		// LDP Maverick cannot gaurantee the exit code until session
