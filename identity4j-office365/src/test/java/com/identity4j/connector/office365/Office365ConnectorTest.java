...
import com.identity4j.connector.office365.Office365Identity;
 ...
 * 
 * There is Pre Requisite requirement of having a valid user entry in google user data store
 * That user should be mapped by configuration key for id : connector.validIdentityName and 
 * for password : connector.validIdentityPassword 
 * 
 * For Role there should be entry for name : connector.validRoleName and for email :
...
public class Office365ConnectorTest extends AbstractRestWebServiceConnectorTest{
	...
	public void itShouldThrowConnectorExceptionIfServicePrincipalDoesNotHaveDeletePrivilegeAndDeleteIdentityOperationIsPerformed() throws Exception{
		//given a connector with no delete privilege
		Office365Connector connector = null;
	...
	@Test(expected=ConnectorException.class)
	public void itShouldThrowConnectorExceptionIfServicePrincipalDoesNotHaveDeletePrivilegeAndUpdateIdentityOperationIsPerformed() throws Exception{
		//given a connector with no delete privilege
		Office365Connector connector = null;
	...
	@Test(expected=ConnectorException.class)
	public void itShouldThrowConnectorExceptionIfServicePrincipalDoesNotHaveDeletePrivilegeAndDeleteRoleOperationIsPerformed() throws Exception{
		//given a connector with no delete privilege
		Office365Connector connector = null;
	...
	@Test(expected=ConnectorException.class)
	public void itShouldThrowConnectorExceptionIfServicePrincipalDoesNotHaveDeletePrivilegeAndUpdateRoleOperationIsPerformed() throws Exception{
		//given a connector with no delete privilege
		Office365Connector connector = null;
	...
	public void itShouldThrowPrincipalAlreadyExistsExceptionIfRoleAlreadyPresentInDataSource(){
		...
		role.setAttribute("mailNickname", validRoleEmail.split("@")[0]);
		role.setAttribute("description", validRoleDescription);
		role.setAttribute("mailEnabled", "false");
		role.setAttribute("securityEnabled", "true");
		//when role is created
	...
	public void itShouldCreateIdentityIfNotPresentInDataSourceAlongWithRolesProvided() {
		...
		//and a valid role
		officeIdentity.addRole(connector.getRoleByName(validRoleObjectId));
		try {
			...
			Identity googleIdentityFromSource = connector
			...
			Assert.assertEquals("Principal names should be same",
					officeIdentity.getPrincipalName(),
	...
	public void itShouldUpdateIdentityWithTheChangesPassedSpecificToRole() {
		...
		try {
			...
			officeIdentity.setRoles(new Role[]{connector.getRoleByName(dummy1RoleId),connector.getRoleByName(dummy2RoleId)});
			...
			Assert.assertEquals("Full name should be same as updated name",
					officeIdentity.getFullName(),
					officeIdentityFromSource.getFullName());
			Assert.assertEquals("Total Roles assigned should be 2", 2, officeIdentityFromSource.getRoles().length);
	...
	private void assertRoleIsPresent(Role[] roles, String principalName){
		for (Role role : roles) {
			if(role.getGuid().equals(principalName)) return;
		}
